  ...
  Future<void> getMembers() async {
    ...
    for (int i = 0; i < userMembers.length; i++) {
      setState(() {
        members.add({
          "id": userMembers[i]["id"],
          "name": userMembers[i]["name"],
          "level": userMembers[i]["level"],
          "rarity": userMembers[i]["rarity"],
          "status": userMembers[i]["status"],
          "exp": userMembers[i]["exp"] ?? 0,
          "crtHp": userMembers[i]["status"]["hp"],
          "image": userMembers[i]["image"],
        });
      });
    }
  }
                      ...
                      Column(
                        children: [
                          BattleController(
                              ...
                              playerFullHp: members[currentTurn]["status"]
                                  ["hp"],
